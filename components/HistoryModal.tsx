...
export const HistoryModal: React.FC<HistoryModalProps> = ({ messages, onClose }) => {
  return (
    ...
    >
      <div 
        className="bg-gray-800 border border-indigo-500/50 rounded-lg shadow-2xl max-w-2xl w-full relative flex flex-col max-h-[90vh]"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="p-4 border-b border-indigo-500/30 flex justify-between items-center flex-shrink-0">
          <h2 style={{ fontFamily: 'Cinzel, serif' }} className="text-2xl font-bold text-purple-300">
            Chat History
          </h2>
          ...
          {messages.length === 0 ? (
            ...
            messages.map(msg => (
              <div key={msg.id} className={`flex ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`text-sm p-2 rounded-lg max-w-[80%] ${msg.sender === 'user' ? 'bg-indigo-600' : 'bg-gray-700'}`}>
